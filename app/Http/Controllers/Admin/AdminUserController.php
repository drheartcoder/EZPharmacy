...
class AdminUserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($user)
        {
            ...
            $data = DB::table('users as US')->where('US.email','=',$arr_data['email'])->leftjoin('role_users as RU','US.id','=','user_id')->first();

            if(isset($data->role_id) && $data->role_id == 1)
            {
               $user_mail = $this->admin_added_mail($arr_data);
            }
            else
            {
    ...
    public function operator_added_mail($data_array)
    {
      ...
      public function admin_added_mail($data_array)
    {

        $selectCols = array('ETT.template_html','ETT.template_subject');

        $whrCondition   =   array('ET.id' => 17);
        $orderBy = $orderStyle = $groupBy = '';
