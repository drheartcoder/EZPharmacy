...
use App\Models\BrochureOptionsModel;  
...
class BrochureOptionsController extends Controller
{
    use MultiActionTrait;

    public function __construct(
                                    BrochureOptionsModel $brochure_options,
                                    ColorModel $color,
                                    LanguageService $langauge
                                )
    {      
       $this->BrochureOptionsModel = $brochure_options;
       $this->BaseModel            = $this->BrochureOptionsModel;

       $this->LanguageService    = $langauge;
       $this->module_title       = "Brochure Option";
       $this->module_icon        = "fa-envelope-o";
       $this->module_view_folder = "admin.brochure_options";
       $this->module_url_path    = url(config('app.project.admin_panel_slug')."/brochure_options");
    }
    ...
    {   
        ...
        if($request->hasFile('image'))
        {
            ...
            if(in_array($file_extension,['png','jpg','jpeg','gif']))
            {
                    ...
                    $isUpload = $request->file('image')->move('uploads/brochure_options/', $file_name);
                    if($isUpload)
                    {
                        $res= $this->attachmentThumb(file_get_contents('uploads/brochure_options/'.$file_name), $file_name, 270, 190);

                        $res1= $this->attachmentThumb(file_get_contents('uploads/brochure_options/'.$file_name), $file_name, 150, 173);
                    }
    ...
    public function update(Request $request)
    {
        ...
        if($request->hasFile('image'))
        {
            ...
            if(in_array($file_extension,['png','jpg','jpeg','gif']))
            {
                    ...
                    if($isUpload)
                    {
                        @unlink('uploads/brochure_options/'.$oldImage);
                        @unlink('uploads/brochure_options/thumb_270X190_'.$oldImage);
                        @unlink('uploads/brochure_options/thumb_150X173_'.$oldImage);
    ...
    public function attachmentThumb($input, $name, $width, $height)
    {
        ...
        $thumb_img->save('uploads/brochure_options/thumb_'.$width.'X'.$height.'_'.$name);
