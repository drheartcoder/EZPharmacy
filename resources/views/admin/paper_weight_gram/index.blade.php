         ...
         <div class="box-content">
            ...
            <form name="frm_manage" id="frm_manage" method="POST" class="form-horizontal" action="{{$module_url_path}}/multi_action">
               ...
               <div class="table-responsive" style="border:0">
                  ...
                  <table class="table table-advance" <?php if(count($weight_gram_arr) > 0){ ?> id="table1" <?php } ?>  >
                     <thead>
                        <tr>
                     ...
                     <tbody>
                        @if(isset($weight_gram_arr) && sizeof($weight_gram_arr)>0)
                          @foreach($weight_gram_arr as $data)
                          <tr>
                             <td><input type="checkbox" name="checked_record[]" value="{{ base64_encode($data->primaryKey) }}" /></td>
                             <td>{{ isset($data->weight_in_grams)?$data->weight_in_grams:'' }}</td>
                             <td>{{ isset($data->size_name)?$data->size_name:'' }}</td>
                             <td>{{ isset($data->weight_in_gsm)?$data->weight_in_gsm:'' }}</td>
                             <td>
