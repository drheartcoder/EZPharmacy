...
<div id="breadcrumbs">
   <ul class="breadcrumb">
      ...
      <a href="{{ $module_url_path }}/manage">Manage Papers Printing</a>
      </span> 
      <span class="divider">
      <i class="fa fa-angle-right"></i>
            ...
            <div class="tabbable">
               <form name="validation-form" id="validation-form" method="POST" action="{{$module_url_path}}/update" class="form-horizontal" enctype="multipart/form-data">
                      ...
                      <select class="form-control" name="category" id="category" disabled="">
                          ...
                          @if(count($category_arr) > 0)
                            @foreach($category_arr as $category_arr_value)
                              <option value="{{ $category_arr_value->id or '' }}" <?php if(isset($price_paper_printing->category_id)){ if($price_paper_printing->category_id == $category_arr_value->id){ echo "Selected=selected"; } else{ echo ""; } } else{ echo ""; } ?> >{{ $category_arr_value->name or 'N/A' }}</option>
                            @endforeach
                          @endif
                        </select>
                      </div>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-sm-6 col-lg-4 controls">
                        <select id="paperSize" name="paperSize" class="form-control" disabled="">
                            @if(count($resPaperSize))
                              <option value="">Select Size</option>
                              @foreach($resPaperSize as $row)
                                <option value="{{$row->primaryKey}}"
                                  {{ isset($price_paper_printing->size_id) && $price_paper_printing->size_id == $row->primaryKey ? 'selected=selected' : '' }}
                                >{{$row->paperSize}}</option>
                              @endforeach
                            @endif
                        </select>
                      </div>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-sm-6 col-lg-4 controls">
                        <select class="form-control" id="paperColor" name="paperColor" disabled="">
                              @if(count($resPaperColor))
                                <option value="">Select Colour</option>
                                @foreach($resPaperColor as $row)
                                  <option value="{{$row->primaryKey}}"
                                    {{ isset($price_paper_printing->colour_id) && $price_paper_printing->colour_id == $row->primaryKey ? 'selected=selected' : '' }}
                                  >{{$row->paperColor}}</option>
                                @endforeach
                              @endif
                          </select>
                      </div>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-sm-6 col-lg-4 controls">   
                        <input type="text" name="price" id="price"  placeholder="Enter Price" value="<?php if(isset($price_paper_printing->price) && $price_paper_printing->price != ''){ echo $price_paper_printing->price; } else { echo ""; } ?>"  class="form-control add-stundt"/>
                      </div>
