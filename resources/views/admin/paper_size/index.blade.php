         ...
         <div class="box-content">
            ...
            <form name="frm_manage" id="frm_manage" method="POST" class="form-horizontal" action="{{$module_url_path}}/multi_action">
               ...
               <div class="table-responsive" style="border:0">
                  ...
                  <table class="table table-advance" <?php if(count($size_arr) > 0){ ?> id="table1" <?php } ?> >
                     <thead>
                        <tr>
                     ...
                     <tbody>
                        @if(isset($size_arr) && sizeof($size_arr)>0)
                          @foreach($size_arr as $data)
                          <?php
                          $is_used = array();
                          $is_used = \DB::table('order_master as OM')
                                        ->select('paper_size')
                                        ->where('paper_size', '=', $data->primaryKey)
                                        ->get();
                          ?>
                          <tr>
                             <td><input type="checkbox" name="checked_record[]" value="{{ base64_encode($data->primaryKey) }}" /></td>
                             <td>{{ isset($data->size_name)?$data->size_name:'' }}</td>
                             <td>
                             ...
                             <td> 
                                ...
                                @else
                                  <a class="btn btn-sm btn-danger show-tooltip" href="javascript:void(0);" onclick="javascript:swal('Size already used,You can\'t delete it.')" title="Used"><i class="fa fa-close" ></i></a>
                                @endif
                             </td>
                          </tr>
                          @endforeach
                        @else
                          <tr><td style="color: red; font-size: 18px; font-family: arial;" colspan="4">No records found.</td></tr>
                        @endif
                     </tbody>
                  </table>
               </div>
