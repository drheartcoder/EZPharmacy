            ...
            <div class="tabbable">
               <form name="validation-form" id="validation-form" method="POST" action="{{$module_url_path}}/update" class="form-horizontal" enctype="multipart/form-data">
                  ...
                  <div class="form-group">
                      ...
                      <input  type="hidden" readonly="" id="category_id" name="category_id" value="{{$price_paper_binding[0]->category_id}}" ></input>  
                      <select class="form-control" name="category" id="category" disabled="">
                          <option value="">Select Category</option>
                          @if(count($category_arr) > 0)
                            @foreach($category_arr as $category_arr_value)
                              <option value="{{ $category_arr_value->id or '' }}" <?php if(isset($price_paper_binding[0]->category_id)){ if($price_paper_binding[0]->category_id == $category_arr_value->id){ echo "Selected=selected"; } else{ echo ""; } } else{ echo ""; } ?> >{{ $category_arr_value->name or 'N/A' }}</option>
                            @endforeach
                          @endif
                        </select>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-sm-6 col-lg-4 controls">   
                       <input  type="hidden" readonly="" id="binding_id" name="binding_id" value="{{$price_paper_binding[0]->binding_id}}" ></input>  
                        <select class="form-control" name="binding" id="binding" disabled=''>
                          <option value="">Select Brochure</option>
                          @if(count($binding_arr) > 0)
                            @foreach($binding_arr as $binding_arr_value)
                              <option value="{{ $binding_arr_value->id or '' }}" <?php if(isset($price_paper_binding[0]->binding_id) && $price_paper_binding[0]->binding_id != ''){ if($price_paper_binding[0]->binding_id == $binding_arr_value->id){ echo "Selected=selected"; } else{ echo ""; } } else{ echo ""; } ?> >{{ $binding_arr_value->option_name or 'N/A' }}</option>
                            @endforeach
                          @endif
                        </select>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-sm-6 col-lg-4 controls">   
                        <input type="text" name="price" id="price"  placeholder="Enter Price" value="<?php if(isset($price_paper_binding[0]->price) && $price_paper_binding[0]->price != ''){ echo $price_paper_binding[0]->price; } else { echo ""; } ?>"  class="form-control add-stundt"/>
                      </div>
